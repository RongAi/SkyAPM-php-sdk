...
ZEND_BEGIN_ARG_INFO(arginfo_skywalking_get_trace_info,
0)
...
ZEND_API void sky_execute_ex(zend_execute_data *execute_data) {
    ...
    if (operationName != NULL) {
        ...
        if (strcmp(class_name, "Predis\\Client") == 0 && strcmp(function_name, "executeCommand") == 0) {
            ...
            if (connection != NULL && Z_TYPE_P(connection) == IS_OBJECT &&
                strcmp(sky_get_class_name(connection), "Predis\\Connection\\StreamConnection") == 0) {
                zval *parameters = sky_read_property(connection, "parameters");
                if (parameters != NULL && Z_TYPE_P(parameters) == IS_OBJECT &&
                    strcmp(sky_get_class_name(parameters), "Predis\\Connection\\Parameters") == 0) {
                    ...
                    if (Z_TYPE_P(parameters_arr) == IS_ARRAY) {
                        ...
                        if (Z_TYPE_P(predis_host) == IS_STRING && Z_TYPE_P(predis_port) == IS_LONG) {
                            ...
                            sprintf(peer, "%s:%"
                            PRId3264, host, Z_LVAL_P(predis_port));
                        }
                    }
                }
            }
        ...
        if (strcmp(class_name, "PDO") == 0) {
            ...
            if (dbh != NULL) {
                ...
                if (dbh->data_source != NULL && db_type[0] != '\0') {
                    ...
                    char *host = pcre_match("(host=([^;\\s]+))", sizeof("(host=([^;\\s]+))") - 1,
                                            (char *) dbh->data_source);
                    char *port = pcre_match("(port=([^;\\s]+))", sizeof("(port=([^;\\s]+))") - 1,
                                            (char *) dbh->data_source);
...
void log_message(char *message) {
    FILE *log;
    log = fopen("/data1/www/applogs/skywalking/php-ext.log", "a");
    fprintf(log, "%s\n", message);
    fclose(log);
}
...
static char *generate_sw3(zend_long span_id, char *peer_host, char *operation_name) {
    ...
    ssize_t sw3_l = 0;
    sw3_l = snprintf(NULL, 0, "sw3: %s|%"
    PRId3264
    "|%d|%"
    PRId3264
    "|#%s|#%s|#%s|%s", Z_STRVAL_P(traceId), span_id,
...
static char *generate_sw6(zend_long span_id, char *peer_host, char *operation_name) {
    ...
    ssize_t sw6_l = 0;
    sw6_l = snprintf(NULL, 0, "sw6: 1-%s-%s-%"
    PRId3264
    "-%d-%"
    PRId3264
    "-%s-%s-%s", Z_STRVAL(distributedTraceIdEncode),
    ...
    bzero(sw6, sw6_l + 1);
    snprintf(sw6, sw6_l + 1, "sw6: 1-%s-%s-%"
    PRId3264
    "-%d-%"
    PRId3264
    "-%s-%s-%s", Z_STRVAL(distributedTraceIdEncode),
    ...
    if (SKYWALKING_G(version) == 5) {
        ...
        if (sw != NULL && Z_TYPE_P(sw) == IS_STRING && Z_STRLEN_P(sw) > 10) {
            ...
            if (zend_array_count(Z_ARRVAL_P(&temp)) >= 8) {
                ...
                add_assoc_long(&SKYWALKING_G(context), "parentSpanId",
                               zend_atol(Z_STRVAL_P(sw3_1), sizeof(Z_STRVAL_P(sw3_1)) - 1));
                add_assoc_long(&SKYWALKING_G(context), "parentApplicationInstance",
                               zend_atol(Z_STRVAL_P(sw3_2), sizeof(Z_STRVAL_P(sw3_2)) - 1));
                add_assoc_long(&SKYWALKING_G(context), "entryApplicationInstance",
                               zend_atol(Z_STRVAL_P(sw3_3), sizeof(Z_STRVAL_P(sw3_3)) - 1));
        ...
        if (sw != NULL && Z_TYPE_P(sw) == IS_STRING && Z_STRLEN_P(sw) > 10) {
            ...
            if (zend_array_count(Z_ARRVAL_P(&temp)) >= 7) {
                ...
                add_assoc_long(&SKYWALKING_G(context), "parentSpanId",
                               zend_atol(Z_STRVAL_P(sw6_3), sizeof(Z_STRVAL_P(sw6_3)) - 1));
                add_assoc_long(&SKYWALKING_G(context), "parentApplicationInstance",
                               zend_atol(Z_STRVAL_P(sw6_4), sizeof(Z_STRVAL_P(sw6_4)) - 1));
                add_assoc_long(&SKYWALKING_G(context), "entryApplicationInstance",
                               zend_atol(Z_STRVAL_P(sw6_5), sizeof(Z_STRVAL_P(sw6_5)) - 1));
...
unsigned int BkdrHash(const char *str) {
    ...
    char *result = malloc(strlen(str) + strlen(current_ip) + 1);//+1 for the zero-terminator
    //in real code you would check for errors in malloc here
    if (result == NULL) return -1;

    strcpy(result, str);
    strcat(result, current_ip);

    unsigned int len = sizeof(result);

    for (i = 0; i < len; ++i) {
        hash = (hash * seed) + (*result++);
    }
...
static void request_init() {
    ...
    add_assoc_long(&SKYWALKING_G(UpstreamSegment), "pid", BkdrHash(SKYWALKING_G(app_code)));
    ...
    if (Z_LVAL_P(isChild) == 1) {
        zval ref;
        array_init(&ref);
        zval *parentTraceSegmentId = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)), "parentTraceSegmentId",
                                                        sizeof("parentTraceSegmentId") - 1);
        zval *parentSpanId = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)), "parentSpanId",
                                                sizeof("parentSpanId") - 1);
        zval *parentApplicationInstance = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)),
        ...
        zval *entryOperationName = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)), "entryOperationName",
                                                      sizeof("entryOperationName") - 1);
        zval *networkAddress = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)), "networkAddress",
                                                  sizeof("networkAddress") - 1);
        zval *parentOperationName = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)), "parentOperationName",
                                                       sizeof("parentOperationName") - 1);
        zval *distributedTraceId = zend_hash_str_find(Z_ARRVAL(SKYWALKING_G(context)), "distributedTraceId",
                                                      sizeof("distributedTraceId") - 1);
    ...
    zend_hash_str_update(Z_ARRVAL(SKYWALKING_G(UpstreamSegment)), "segment", sizeof("segment") - 1,
                         &traceSegmentObject);
    zend_hash_str_update(Z_ARRVAL(SKYWALKING_G(UpstreamSegment)), "globalTraceIds", sizeof("globalTraceIds") - 1,
                         &globalTraceIds);
}
    ...
    if (application_instance == 0) {
        ...
        if (fd >= 0) {
            ...
            if (conn >= 0) {
                ...
                sprintf(message, "0{\"app_code\":\"%s\",\"pid\":%ld,\"version\":%d}\n", SKYWALKING_G(app_code),
                        BkdrHash(SKYWALKING_G(app_code)), SKYWALKING_G(version));
                write(fd, message, strlen(message));
                ...
                char *errormessage;
                sprintf(errormessage, "[skywalking] failed to connect the sock. error: %s", strerror(errno));
                php_error_docref(NULL, E_WARNING, errormessage);
                efree(errormessage);
            }
            ...
            char *errormessage;
            sprintf(errormessage, "[skywalking] failed to open the sock. error: %s", strerror(errno));
            php_error_docref(NULL, E_WARNING, errormessage);
            efree(errormessage);
        }
    }
...
PHP_MINIT_FUNCTION (skywalking) {
        ...
        */
        if (SKYWALKING_G(enable)) {
            ...
            current_ip = _get_current_machine_ip();
            ...
            if ((old_function = zend_hash_str_find_ptr(CG(function_table), "curl_exec", sizeof("curl_exec") - 1)) !=
                NULL) {
                orig_curl_exec = old_function->internal_function.handler;
                old_function->internal_function.handler = sky_curl_exec_handler;
            }
            if ((old_function = zend_hash_str_find_ptr(CG(function_table), "curl_setopt", sizeof("curl_setopt") - 1)) !=
                NULL) {
                orig_curl_setopt = old_function->internal_function.handler;
                old_function->internal_function.handler = sky_curl_setopt_handler;
            }
            if ((old_function = zend_hash_str_find_ptr(CG(function_table), "curl_setopt_array",
                                                       sizeof("curl_setopt_array") - 1)) != NULL) {
            ...
            if ((old_function = zend_hash_str_find_ptr(CG(function_table), "curl_close", sizeof("curl_close") - 1)) !=
                NULL) {
